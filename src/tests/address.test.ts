...
describe('Testing MultiSig Address', () => {
  describe('[POST] /address/multisig', () => {
    it('test happy path - responds with expected address', () => {
      ...
      return request(app.getServer()).post(`/address/multisig`).send(data).expect(200, { address: '3DzZeJHxdCgWnDCjwmJZap1nHNLJNvWvGJ' });
    });
    ...
    it('returns an error as n > m', () => {
      ...
      return request(app.getServer())
        .post(`/address/multisig`)
        .send(data)
        .expect(500, { message: 'Pubkey count cannot be less than m' });
    });
